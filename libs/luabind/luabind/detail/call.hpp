...
namespace luabind {
	namespace detail {
		...
		struct match_struct {
			...
			{
				const int this_match = std::get<Index - 1>(tuple).match(L, decorate_type_t<typename SignatureList::template at<Index>>(), luabind::meta::get<StackIndexList, Index - 1>::value);
